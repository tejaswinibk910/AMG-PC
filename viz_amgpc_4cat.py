...
ckpts = [f for f in glob.glob('./log/AMG_PC_*/ckpt_*.pt') if 'uniform' not in f]
ckpt_path = min(ckpts, key=lambda f: float(re.search(r'ckpt_\d+_([\d.]+)\.pt', f).group(1)))
epoch_num = int(re.search(r'ckpt_(\d+)_', ckpt_path).group(1))
cd_val = float(re.search(r'ckpt_\d+_([\d.]+)\.pt', ckpt_path).group(1))
print(f'Using: {ckpt_path}  (epoch {epoch_num}, CD {cd_val:.4f})')
...
def load_and_run(cat_id, obj_id, view_id):
    ...
    for p in [pc_part_path, pc_path, view_path, meta_path]:
    ...
    try:
        ...
        vid = int(view_id)
        phi_part   = math.radians(meta[vid, 1])
        theta_part = math.radians(meta[vid, 0])
        phi_img    = math.radians(meta[vid, 1])
        theta_img  = math.radians(meta[vid, 0])
        ...
        pred_t = torch.from_numpy(pred_np.copy()).float().unsqueeze(0).to(device)
        gt_t   = torch.from_numpy(pc.copy()).float().unsqueeze(0).to(device)
        cd = loss_cd(pred_t, gt_t).item() * 1e3

        return {'pc_part': pc_part, 'pc': pc, 'pred_np': pred_np, 'gates': gates, 'cd': cd}
    except:
        return None
...
samples = []
with torch.no_grad():
    for cat_id, label, keep, n_candidates in CATS:
        ...
        for obj_id in objs:
            if len(candidates) >= n_candidates:
                break
            view_id = str(random.randint(0, 23)).zfill(2)
            result = load_and_run(cat_id, obj_id, view_id)
            if result is not None:
                candidates.append(result)
                print(f'  [{label}] candidate {len(candidates)}/{n_candidates} CD={result["cd"]:.4f}')

        if not candidates:
            print(f'No valid candidates for {label}')
            continue

        candidates.sort(key=lambda x: x['cd'])
        best = candidates[:keep]

        for i, r in enumerate(best):
            samples.append({
                'label':   f'{label} #{i+1}  CD={r["cd"]:.3f}',
                'partial': r['pc_part'].tolist(),
                'pred':    r['pred_np'].tolist(),
                'gt':      r['pc'].tolist(),
                'gates':   r['gates'],
            })
            print(f'Selected: {label} #{i+1} CD={r["cd"]:.4f}')
...
<head>
<meta charset="utf-8">
<title>AMG-PC 4-Category Visualization</title>
...
<style>
  ...
  *{{margin:0;padding:0;box-sizing:border-box;}}
  ...
  .logo h1{{font-size:22px;font-weight:600;letter-spacing:-0.03em;}}
  ...
  .stat-box{{padding:10px 20px;border-right:1px solid var(--border);text-align:center;}}
  ...
  canvas{{display:block;width:100%;height:260px;cursor:grab;}}
...
<header>
  <div class="logo">
    <h1>AMG-PC <span>4-Category</span> Visualization</h1>
    <p>Epoch {epoch_num} &nbsp;·&nbsp; Plane · Car · Chair &nbsp;·&nbsp; ShapeNet-ViPC &nbsp;·&nbsp; Best 2 per category by CD</p>
  </div>
  <div class="stats">
    <div class="stat-box"><span class="stat-val">{cd_val:.4f}</span><span class="stat-lbl">CD ×10⁻³</span></div>
    <div class="stat-box"><span class="stat-val">{epoch_num}</span><span class="stat-lbl">Epoch</span></div>
    <div class="stat-box"><span class="stat-val">2048</span><span class="stat-lbl">Points</span></div>
    <div class="stat-box"><span class="stat-val">3</span><span class="stat-lbl">Categories</span></div>
  </div>
</header>
<div class="legend">
...
const LBLS = ['partial','pred','gt'];
const LNAMES = ['Partial Input','Predicted','Ground Truth'];
const LTAGS = ['tag-p','tag-r','tag-g'];
const MOD_LABELS = ['PC','IMG','TXT'];
const MOD_COLORS = ['#58a6ff','#ffa657','#a78bfa'];
...
function norm(pts){{
  ...
  let mx=0;
  pts.forEach(p=>{{const d=Math.hypot(p[0]-cx,p[1]-cy,p[2]-cz);if(isFinite(d)&&d>mx)mx=d;}});
  if(mx<1e-6)mx=1;
  return pts.map(p=>{{
    const x=(p[0]-cx)/mx,y=(p[1]-cy)/mx,z=(p[2]-cz)/mx;
    return [isFinite(x)?x:0,isFinite(y)?y:0,isFinite(z)?z:0];
  }});
}}

function makePC(pts,color,big){{
  const g=new THREE.BufferGeometry();
  const n=norm(pts);
  const pos=new Float32Array(n.length*3);
  n.forEach((p,i)=>{{pos[i*3]=p[0];pos[i*3+1]=p[1];pos[i*3+2]=p[2];}});
  g.setAttribute('position',new THREE.BufferAttribute(pos,3));
  return new THREE.Points(g,new THREE.PointsMaterial({{color,size:big?0.05:0.038,transparent:true,opacity:0.95,sizeAttenuation:true,depthWrite:false}}));
}}

function makeScene(pts,color,big){{
  const s=new THREE.Scene();s.background=new THREE.Color(0x0c0c18);
  s.add(makePC(pts,color,big));return s;
}}

function makeCamera(){{
  const c=new THREE.PerspectiveCamera(40,1,0.01,100);
  c.position.set(2.6,1.6,2.6);c.lookAt(0,0,0);return c;
}}

class Orbit{{
  constructor(cam,el){{
    this.cam=cam;this.theta=0.75;this.phi=0.62;this.r=3.6;
    this.down=false;this.lx=0;this.ly=0;this.cb=null;
    el.addEventListener('mousedown',e=>{{this.down=true;this.lx=e.clientX;this.ly=e.clientY;e.preventDefault();}});
    window.addEventListener('mousemove',e=>{{
      if(!this.down)return;
      const dx=e.clientX-this.lx,dy=e.clientY-this.ly;
      this.lx=e.clientX;this.ly=e.clientY;
      this.theta-=dx*0.004;
      this.phi=Math.max(0.08,Math.min(Math.PI-0.08,this.phi-dy*0.004));
      this._u();if(this.cb)this.cb(this.theta,this.phi,this.r);
    }});
    window.addEventListener('mouseup',()=>{{this.down=false;}});
    el.addEventListener('wheel',e=>{{
      this.r=Math.max(1,Math.min(9,this.r+e.deltaY*0.005));
      this._u();if(this.cb)this.cb(this.theta,this.phi,this.r);
      e.preventDefault();
    }},{{passive:false}});
    this._u();
  ...
  _u(){{
    ...
    this.cam.position.set(this.r*s*Math.sin(this.theta),this.r*Math.cos(this.phi),this.r*s*Math.cos(this.theta));
    this.cam.lookAt(0,0,0);
  }}
  sync(t,p,r){{this.theta=t;this.phi=p;this.r=r;this._u();}}
}}

function makeGatePanel(gates){{
  const div=document.createElement('div');div.className='gate-panel';
  const title=document.createElement('div');title.className='gate-title';title.textContent='Modality Gate Weights';
  div.appendChild(title);
  gates.forEach((gw,si)=>{{
    const sl=document.createElement('div');sl.className='gate-stage-label';sl.textContent=STAGE_LABELS[si];div.appendChild(sl);
    gw.forEach((w,mi)=>{{
      const row=document.createElement('div');row.className='gate-bar-row';
      const lbl=document.createElement('div');lbl.className='gate-bar-label';lbl.textContent=MOD_LABELS[mi];
      const bg=document.createElement('div');bg.className='gate-bar-bg';
      const fill=document.createElement('div');fill.className='gate-bar-fill';
      fill.style.width=`${{(w*100).toFixed(1)}}%`;fill.style.background=MOD_COLORS[mi];
      bg.appendChild(fill);
      const val=document.createElement('div');val.className='gate-bar-val';val.textContent=w.toFixed(3);
      row.appendChild(lbl);row.appendChild(bg);row.appendChild(val);div.appendChild(row);
    }});
  }});
  return div;
}}
...
SAMPLES.forEach((sam,si)=>{{
  ...
  hdr.innerHTML=`<span class="card-idx">#${{String(si+1).padStart(2,'0')}}</span><span class="card-name">${{sam.label}}</span>`;
  card.appendChild(hdr);
  const body=document.createElement('div');body.className='card-body';card.appendChild(body);cont.appendChild(card);
  LBLS.forEach((lbl,li)=>{{
    const wrap=document.createElement('div');wrap.className='cv-wrap';
    const tag=document.createElement('div');tag.className=`cv-tag ${{LTAGS[li]}}`;tag.textContent=LNAMES[li];
    wrap.appendChild(tag);
    const canvas=document.createElement('canvas');wrap.appendChild(canvas);body.appendChild(wrap);
    const r=new THREE.WebGLRenderer({{canvas,antialias:true}});
    r.setPixelRatio(Math.min(window.devicePixelRatio,2));
    const sc=makeScene(sam[lbl],COLS[lbl],lbl==='partial');
    const cam=makeCamera();
    const orb=new Orbit(cam,canvas);
    allO.push(orb);allR.push(r);allS.push(sc);allC.push(cam);
    orb.cb=(t,p,rv)=>allO.forEach(o=>{{if(o!==orb)o.sync(t,p,rv);}});
  }});
  body.appendChild(makeGatePanel(sam.gates));
}});

function resize(){{
  allR.forEach((r,i)=>{{
    const c=r.domElement,w=c.clientWidth,h=c.clientHeight;
    if(c.width!==w||c.height!==h){{r.setSize(w,h,false);allC[i].aspect=w/h;allC[i].updateProjectionMatrix();}}
  }});
}}
...
with open('/tmp/amgpc_viz.html', 'w') as f:
    f.write(html)
print('Saved: /tmp/amgpc_viz.html')
