...
from config_vipc_uniform import params
from models.AMG_PC_uniform import AMG_PC
...
def main():
    ...
    if cfg.resume:
        import glob, re
        all_ckpts = glob.glob('./log/AMG_PC_uniform_*/ckpt_*.pt')
        valid_ckpts = []
        for f in all_ckpts:
            try:
                torch.load(f, map_location='cpu')
                valid_ckpts.append(f)
            except: pass
        if valid_ckpts:
            cfg.ckpt = min(valid_ckpts, key=lambda f: float(re.search(r'ckpt_\d+_([\d.]+)\.pt', f).group(1)))
    ...
    gate_log_path = '/scratch/tbalamur/gate_weights_uniform.txt'
