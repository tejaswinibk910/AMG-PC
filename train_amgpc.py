...
from dataloader_amgpc import ViPCDataLoader
...
def main():
    ...
    if cfg.resume:
        import glob, re
        all_ckpts = glob.glob('./log/AMG_PC_[0-9]*/*.pt') + glob.glob('./log/AMG_PC_six*/*.pt') + glob.glob('./log/AMG_PC_four*/*.pt') + glob.glob('./log/AMG_PC_all*/*.pt') + glob.glob('./log/AMG_PC_car*/*.pt')
        # exclude uniform ablation checkpoints
        all_ckpts = [f for f in all_ckpts if 'uniform' not in f]
        if all_ckpts:
            cfg.ckpt = min(all_ckpts, key=lambda f: float(re.search(r'ckpt_\d+_([\d.]+)\.pt', f).group(1)))
            print(f'Auto-resuming from: {cfg.ckpt}')
        else:
    ...
    # Gate weight log file — records per-stage modality weights every 100 batches
    gate_log_path = os.path.join(log_dir, 'gate_weights.txt')
    ...
    for epoch in range(resume_epoch, cfg.n_epochs + 1):
        ...
        with tqdm(train_loader) as t:
            for batch_idx, data in enumerate(t):
                ...
                text_embed = data[3].to(device)   # (B, 512) — new

                out, gate_weights = model(partial, image, text_embed)
                ...
                t.set_postfix(loss='%s' % ['%.4f' % l for l in [
                    1e3 * loss_stage0.data.cpu(),
                    1e3 * loss_stage1.data.cpu(),
                    1e3 * loss_stage2.data.cpu(),
                    1e3 * loss_stage3.data.cpu()
                ]])

                # Log gate weights every 100 batches for analysis
                if batch_idx % 100 == 0:
                    with open(gate_log_path, 'a') as gf:
                        for stage_i, gw in enumerate(gate_weights):
                            mean_w = gw.mean(dim=0).detach().cpu().numpy()
                            gf.write(f'epoch={epoch} batch={batch_idx} '
                                     f'stage={stage_i} '
                                     f'pc={mean_w[0]:.4f} '
                                     f'img={mean_w[1]:.4f} '
                                     f'txt={mean_w[2]:.4f}\n')
            ...
            with torch.no_grad():
                ...
                with tqdm(test_loader) as t:
                    for batch_idx, data in enumerate(t):
                        ...
                        gt         = data[1].to(device)
                        partial    = data[2].to(device)
                        text_embed = data[3].to(device)

                        out, _ = model(partial, image, text_embed)
                    ...
                    if Loss < best_loss:
                        ...
                        torch.save({
                            ...
                            'loss': Loss},
                            f'{log_dir}/ckpt_{epoch}_{Loss:.4f}.pt')
